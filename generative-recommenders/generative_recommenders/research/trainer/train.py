...
def train_fn(
    ...
    if torch.cuda.is_available():
        device = rank  # Use GPU with id rank
    else:
        device = "cpu"  # Use CPU
    
    if main_module_bf16 and torch.cuda.is_available():
        model = model.to(torch.bfloat16)
    model = model.to(device)
    ar_loss = ar_loss.to(device)
    negatives_sampler = negatives_sampler.to(device)
    
    # Wrap with DDP (works for both single and multi-process)
    if torch.cuda.is_available():
    ...
    if enable_profiling and rank == 0:
        profiler_dir = f"./profiler/{model_desc}"
        ...
        profiler = profile(
            ...
            with_flops=True,
            with_modules=True,
            execution_trace_observer=(ExecutionTraceObserver().register_callback(f"{profiler_dir}/execution_trace.json")
    )
        )
        logging.info(f"Profiler enabled. Traces will be saved to {profiler_dir}")
        logging.info(f"Profiler schedule: warmup={profile_warmup_steps}, active={profile_active_steps}, repeat={profile_repeat_steps}")
    ...
    for epoch in range(num_epochs):
        ...
        for row in iter(train_data_loader):
            # Start profiling step if enabled
    ...
    if rank == 0:
        ...
        if profiler is not None:
            profiler.stop()
            profiler.export_memory_timeline(f"{profiler_dir}/memory_timeline.html")
            logging.info("Profiler stopped. Check TensorBoard for profiling results.")
