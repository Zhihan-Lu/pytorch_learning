...
def _hstu_attention_maybe_from_cache(
    num_heads: int, # 1
    attention_dim: int, # dqk = 50
    linear_dim: int, # dv = 50
    q: torch.Tensor, # (BxN, attention_dim * num_heads)
    k: torch.Tensor, # (BxN, attention_dim * num_heads)
    v: torch.Tensor, # (BxN, linear_dim * num_heads)
    cached_q: Optional[torch.Tensor],
    cached_k: Optional[torch.Tensor],
    delta_x_offsets: Optional[Tuple[torch.Tensor, torch.Tensor]],
    x_offsets: torch.Tensor, # (B + 1) x int32
    all_timestamps: Optional[torch.Tensor], # (B, N) x int64
    invalid_attn_mask: torch.Tensor, # (B, N, N) x float
    ...
    attn_output = torch.ops.fbgemm.dense_to_jagged(
        torch.einsum(
            "bhnm,bmhd->bnhd", # out[bi, ni, hi, di] = sum_mj qk_attn[bi, hi, ni, mj] * v[bi, mj, hi, di]
            qk_attn,
...
class SequentialTransductionUnitJagged(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        self._embedding_dim: int = embedding_dim # 50
        self._linear_dim: int = linear_hidden_dim # dv = 50
        self._attention_dim: int = attention_dim # dqk = 50
        self._dropout_ratio: float = dropout_ratio # linear_dropout_rate = 0.2
        self._attn_dropout_ratio: float = attn_dropout_ratio # 0.0
        self._num_heads: int = num_heads # 1
        self._rel_attn_bias: Optional[RelativeAttentionBiasModule] = (
            relative_attention_bias_module
        )
        self._normalization: str = normalization # rel_bias
        self._linear_config: str = linear_config # silu
    ...
    def forward(  # pyre-ignore [3]
        ...
        normed_x = self._norm_input(x) # (B x N, D)

        if self._linear_config == "uvqk":
            batched_mm_output = torch.mm(normed_x, self._uvqk) # (BxN, D) @ (D, linear_dim * 2 * num_heads + attention_dim * num_heads * 2)
            if self._linear_activation == "silu":
                batched_mm_output = F.silu(batched_mm_output) # x / (1+exp(-x))
            elif self._linear_activation == "none":
                batched_mm_output = batched_mm_output
            u, v, q, k = torch.split(
                batched_mm_output,
                [
                    self._linear_dim * self._num_heads, # u (BxN, linear_dim * num_heads)
                    self._linear_dim * self._num_heads, # v (BxN, linear_dim * num_heads)
                    self._attention_dim * self._num_heads, # q (BxN, attention_dim * num_heads)
                    self._attention_dim * self._num_heads, # k (BxN, attention_dim * num_heads)
                ],
                dim=1,
            )
        else:
        ...
        if self._normalization == "rel_bias" or self._normalization == "hstu_rel_bias":
            ...
            attn_output, padded_q, padded_k = _hstu_attention_maybe_from_cache( # (BxN, linear_dim * num_heads)
...
class HSTU(SequentialEncoderWithLearnedSimilarityModule):
    ...
    def __init__(
        ...
        # register buffer not trainable, and it's efficient
