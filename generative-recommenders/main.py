...
# Import fbgemm_gpu only if available (GPU-specific library)
try:
    import fbgemm_gpu  # noqa: F401, E402
except ImportError:
    print("fbgemm_gpu not available - running without GPU optimizations")
...
def _main(argv) -> None:  # pyre-ignore [2]
    ...
    if torch.cuda.is_available():
        ...
        print(f"CUDA available. World size: {world_size}")
        ...
        print("CUDA not available. Running on CPU with single process.")
        # Run training on CPU with single process
        ...
        # Call train_fn with CPU settings
